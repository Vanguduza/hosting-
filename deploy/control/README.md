...
2. Copy `.env.example` to `.env` and set a real OIDC issuer, audience and JWKS URL; obtain a JWT for that audience. Create `.secrets/pg_admin`, `.secrets/pg_api`, `.secrets/pg_worker`, and `.secrets/pg_admitter` with independent random passwords (at least 32 characters) and mode 0600. Compose file secrets are local files, not a production secret store.
3. Run `docker compose --env-file .env up --build`. A fresh database creates the restricted API, worker and admission roles and applies SQL migrations `001` through `008` in order. This only occurs on a fresh PostgreSQL volume; apply later migrations with a dedicated migration role on existing volumes before deploying upgraded services.
