...
3. Run `docker compose --env-file .env up --build`. A fresh database creates the restricted API, worker, admission, hook and buildworker roles and applies SQL migrations `001` through `010` in order. This only occurs on a fresh PostgreSQL volume; apply later migrations with a dedicated migration role on existing volumes before deploying upgraded services. The signed hook listens on loopback port 8081; publish it through a TLS ingress if enabling GitHub push builds. See [trusted source builds](../../docs/BUILD_AND_ADMISSION.md).
