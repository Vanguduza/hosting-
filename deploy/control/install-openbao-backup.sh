...
for unit in dial-openbao-backup.service dial-openbao-backup.timer \
            dial-openbao-backup-health.service dial-openbao-backup-health.timer; do
  install -o root -g root -m 0644 "$script_dir/$unit" /etc/systemd/system/
done
systemctl daemon-reload
systemctl enable --now dial-openbao-backup.timer dial-openbao-backup-health.timer
systemctl --no-pager list-timers dial-openbao-backup.timer dial-openbao-backup-health.timer
