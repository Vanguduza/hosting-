"""Durable dedicated Valkey provisioning with OpenBao CAS credentials."""
import http.client
import json
import secrets
import ssl
...
def claim(conn):
    with conn.transaction():
        job = conn.execute(
            "SELECT id,instance_id,attempts FROM hosting.valkey_jobs "
        ...
        if not job:
            return None
        instance = conn.execute("SELECT * FROM hosting.valkey_instances WHERE id=%s", (job["instance_id"],)).fetchone()
        node = conn.execute("SELECT * FROM hosting.nodes WHERE id=%s", (instance["node_id"],)).fetchone()
        attempt = job["attempts"] + 1
        conn.execute("UPDATE hosting.valkey_jobs SET state='RUNNING',attempts=%s,"
                     "lease_until=now()+interval '10 minutes' WHERE id=%s", (attempt, job["id"]))
        conn.execute("UPDATE hosting.valkey_instances SET state='PROVISIONING' WHERE id=%s", (instance["id"],))
...
def finalize(conn, job, instance, attempt, receipt=None, failure=None):
    if receipt and (receipt.get("instance_id") != str(instance["id"]) or
        ...
        raise ValueError("Valkey readiness proof invalid")
    with conn.transaction():
        current = conn.execute("SELECT state,attempts FROM hosting.valkey_jobs WHERE id=%s FOR UPDATE",
                               (job["id"],)).fetchone()
        if current["state"] != "RUNNING" or current["attempts"] != attempt:
            return False
        if receipt:
            conn.execute("UPDATE hosting.valkey_instances SET state='READY',secret_version=1 WHERE id=%s",
                         (instance["id"],))
            conn.execute("UPDATE hosting.valkey_jobs SET state='COMPLETE',lease_until=NULL,receipt=%s::jsonb "
                         "WHERE id=%s", (json.dumps(receipt), job["id"]))
            action = "valkey.ready"
        elif attempt < 3:
            conn.execute("UPDATE hosting.valkey_jobs SET state='PENDING',lease_until=NULL,last_error=%s, "
                         "next_attempt_at=now()+(%s * interval '1 second') WHERE id=%s",
                         (str(failure)[:120], attempt * 15, job["id"]))
            conn.execute("UPDATE hosting.valkey_instances SET state='QUEUED' WHERE id=%s", (instance["id"],))
            return True
        else:
            conn.execute("UPDATE hosting.valkey_jobs SET state='FAILED',lease_until=NULL,last_error=%s WHERE id=%s",
                         (str(failure)[:120], job["id"]))
            conn.execute("UPDATE hosting.valkey_instances SET state='FAILED' WHERE id=%s", (instance["id"],))
            action = "valkey.failed"
...
def recover_expired(conn):
    with conn.transaction():
        expired = conn.execute("SELECT j.id,j.instance_id,i.organization_id,i.requested_by "
                               "FROM hosting.valkey_jobs j JOIN hosting.valkey_instances i ON i.id=j.instance_id "
                               "WHERE j.state='RUNNING' AND j.attempts=3 AND j.lease_until<now() "
                               "FOR UPDATE OF j SKIP LOCKED LIMIT 20").fetchall()
        for row in expired:
            conn.execute("UPDATE hosting.valkey_jobs SET state='FAILED',lease_until=NULL,"
                         "last_error='lease expired' WHERE id=%s", (row["id"],))
            conn.execute("UPDATE hosting.valkey_instances SET state='FAILED' WHERE id=%s", (row["instance_id"],))
            conn.execute("SELECT set_config('hosting.actor_sub',%s,true)", (row["requested_by"],))
            record(conn, row["organization_id"], row["requested_by"], "valkey.lease_expired",
...
def credential_revision(bao, instance_id):
    try:
        ...
        values = {"app_password": secrets.token_urlsafe(48)}
        try:
            ...
            version, values = bao.get(instance_id, version=1)
    if version != 1 or set(values) != {"app_password"}:
        raise RuntimeError("Valkey credential version mismatch")
    return version
...
def provision(node, instance, certificate, bao):
    ...
    try:
        ...
        connection.request("PUT", "/v1/valkey", body=json.dumps(payload, separators=(",", ":")),
        ...
        if response.status != 200 or receipt.get("instance_id") != str(instance["id"]) or \
            ...
            raise RuntimeError("Node Valkey readiness not proven")
        return receipt
    finally:
        connection.close()
