...
The worker leases the oldest outstanding fact per organization for 60 seconds, sends it over verified HTTPS with a 10-second timeout, and records 2xx acknowledgment. A crash after the receiver accepts and before database acknowledgment causes a duplicate; receiver deduplication is mandatory. Errors retry with exponential backoff up to 300 seconds. After ten attempts, including an expired final lease, the record becomes `DEAD` and blocks subsequent events for that tenant, preserving order; other tenants continue. Run `python tools/event_health.py --max-age-seconds 300 --max-pending 1000` with restricted worker database credentials from an operator scheduler: it prints JSON and exits 2 on a dead event or threshold breach. Alert on that exit status and inspect `state`, `last_error`, and `attempts`. Never delete the outbox to clear a backlog.

On the selected operator monitor, configure the root-owned mode-0600 `/etc/dial-hosting/release-health-check.env` from `deploy/control/release-health-check.env.example` with dedicated restricted `hosting_worker` credentials. Set `EVENT_MAX_AGE_SECONDS` and `EVENT_MAX_PENDING` to the alert thresholds. Run `sh deploy/control/install-release-health-check.sh`; it installs `dial-event-outbox-health.timer`, which runs the check each minute. Route failures of `dial-event-outbox-health.service` to an independently supervised alert receiver. The installer does not create that receiver.
