...
`BUILD_READY=false`, `RUNTIME_QUALIFIED=false`, `PRODUCTION_QUALIFIED=false`. The repository contains authenticated tenant/project/application APIs; trusted-source build, scan, sign and digest admission; private release scheduling and an mTLS Docker node agent; rollback and a durable worker with domain proof, Traefik HTTPS routing and public release probes; OpenBao KV v2 credential transport; dedicated private PostgreSQL provisioning and application attachment; and control-database backup/isolated restore drill tooling. Public ingress and database provisioning have code and disposable tests, but no live deployment qualification. GitHub webhook automation, full managed PostgreSQL backup/restore/rotation, managed Supabase, billing, provider IAM setup and the client portal have **not** been implemented or certified. A missing capability is omitted rather than represented by a fake route or green status.
